...
app.post('/api/bookings', async (req, res) => {
  ...
  // Same safety net as Rose's booking tool — the booking.html date picker
  // already excludes Saturdays and specific closed dates, but that's a
  // frontend-only protection. Enforce it here too, since this endpoint is
  // reachable directly.
  const closedReason = officeClosedReason(date);
  if (closedReason) {
    return res.status(400).json({ error: `${closedReason} Please choose a different day.` });
  }
  if (getDayOfWeek(date) === 0 && timeSlotToMinutes(time) >= timeSlotToMinutes('4:00 PM')) {
    return res.status(400).json({ error: "We're only open until 4 PM on Sundays. Please choose an earlier time or a different day." });
  }
...
const VOICE_TIME_SLOTS = ['10:00 AM', '11:00 AM', '12:00 PM', '1:00 PM', '2:00 PM', '3:00 PM', '4:00 PM', '5:00 PM'];

// Specific one-off dates the office is fully closed on top of the regular
// weekly schedule (holidays, staff events, etc.) — add/remove dates here as
// needed, in YYYY-MM-DD format.
const CLOSED_DATES = ['2026-09-21', '2026-09-27', '2026-10-04'];

// Dates the office is open, but every appointment slot is already taken (not
// a closure — just nothing left to book). Callers hear a different message
// for these than for CLOSED_DATES.
const FULLY_BOOKED_DATES = ['2026-09-24'];

// Returns the day of week for a YYYY-MM-DD date string (0=Sunday, 6=Saturday).
// Parsed as UTC since there's no time component — avoids any local-timezone
// off-by-one risk right at a day boundary.
function getDayOfWeek(dateStr) {
  return new Date(dateStr + 'T00:00:00Z').getUTCDay();
}

// Single source of truth for "is this date bookable, and why not." Returns
// null if bookable, or a caller-friendly reason string if not — the wording
// is deliberately different for a genuine closure vs. a day that's open but
// has no slots left, since those aren't the same thing.
function officeClosedReason(dateStr) {
  if (getDayOfWeek(dateStr) === 6) return "We're closed on Saturdays.";
  if (CLOSED_DATES.includes(dateStr)) return `We're closed on ${formatDateLabel(dateStr)}.`;
  if (FULLY_BOOKED_DATES.includes(dateStr)) return `There are no appointments available on ${formatDateLabel(dateStr)}.`;
  return null;
}
...
async function handleCheckAvailability(args) {
  ...
  const closedReason = officeClosedReason(date);
  if (closedReason) {
    return `${closedReason} Would you like me to check a different day?`;
  }
  ...
  // Sunday hours are shorter (10 AM–4 PM) than the weekday range these slots
  // were built for — drop anything at or after 4 PM on Sundays specifically.
  if (getDayOfWeek(date) === 0) {
    available = available.filter(t => timeSlotToMinutes(t) < timeSlotToMinutes('4:00 PM'));
  }
...
async function handleBookAppointment(args) {
  ...
  // Hard safety net — reject a closed date/day here too, not just in
  // check_availability, in case that step got skipped or a caller gave a
  // specific date and time directly. This is the function that actually
  // writes to the database, so this is the last line of defense.
  const closedReason = officeClosedReason(date);
  if (closedReason) {
    return `${closedReason} I can't book that day — would you like a different day instead?`;
  }
  if (getDayOfWeek(date) === 0 && timeSlotToMinutes(time) >= timeSlotToMinutes('4:00 PM')) {
    return `We're only open until 4 PM on Sundays, so that time isn't available — would you like an earlier time or a different day?`;
  }
  ...
  // Don't make Rose wait on two sequential third-party email API calls
  // before she can confirm the booking out loud — that's real, noticeable
  // dead air on a live call. Fire both emails in the background instead;
  // if either fails, it's logged, not spoken to the caller.
  if (email) {
    sendEmail(
      email,
      `Your appointment is confirmed — ${formatDateLabel(date)}`,
  ...
  sendOfficeBookingNotification({ date, time, name, phone, email, reason, dateOfBirth, homeAddress, insuranceProvider, insuranceMemberId }, 'phone — Rose')
    .catch(err => console.error('Office booking notification email failed:', err.message));
